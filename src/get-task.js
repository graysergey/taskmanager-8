...
export const getTask = (taskData) => {
  const color = sortRandomArray([...taskData.color]).slice(0, 1);
  ...
  const repeatDay = Object.keys(taskData.isRepeating);
  ...
  const getHashTagsMarkup = () => {
    let hashTags = sortRandomArray([...taskData.tags]).slice(0, 3)
    .map((it) => {
      return `
  ...
  const getMarkupRepeatingTask = () => {
    return repeatDay.map((day) =>
      `<input
        ...
        ${taskData.isRepeating[day] ? `checked` : ``}
      />
...
export const getTasks = (data, count) => [...Array(count)].map(() => getTask(data)).join(``);
